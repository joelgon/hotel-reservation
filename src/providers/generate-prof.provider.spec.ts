...
describe('GenerateProofProvider', () => {
  ...
  it('should generate a PDF with the correct content', async () => {
    ...
    expect(mockPdfDoc.addPage().drawText).toHaveBeenCalledWith(
      `Nome do Cliente: ${proofPayment.name}`,
      expect.objectContaining({ x: 50, y: 761.89 })
    );
    expect(mockPdfDoc.addPage().drawText).toHaveBeenCalledWith(
      `Valor Total: R$ ${proofPayment.totalValue}`,
      expect.objectContaining({ x: 50, y: 741.89 })
    );
    ...
    expect(mockPdfDoc.addPage().drawText).toHaveBeenCalledWith(
      `Valor da Diária: R$ ${proofPayment.dailyValue} por noite`,
      expect.objectContaining({ x: 70, y: 621.89 })
    );
    expect(mockPdfDoc.addPage().drawText).toHaveBeenCalledWith(
      `Valor Total da Estadia: R$ ${proofPayment.totalValue} (Calculado como ${proofPayment.days} noites x R$ ${proofPayment.dailyValue} por noite)`,
      expect.objectContaining({ x: 50, y: 601.89 })
    );
    expect(mockPdfDoc.addPage().drawText).toHaveBeenCalledWith(
      'Obrigado por escolher nossos serviços. Desejamos a você uma excelente estadia!',
      expect.objectContaining({ x: 50, y: 571.89 })
    );
